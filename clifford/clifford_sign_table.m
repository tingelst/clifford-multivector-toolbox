...
% Copyright (c) 2011, 2014, 2015 Stephen J. Sangwine and Eckhard Hitzer.

% This function is based on the function clifford_tensor, which is how the
...
g = zeros([m, m], 'int8'); % Pre-allocate space for the sign table.
...
for I = uint32(0:m-1) %#ok<*BDSCI>
    for J = uint32(0:m-1)        
        S = int8(1); % This will become the sign (+1, 0, or -1 ultimately).
        ...
        for t = uint32(1:n) % Bits in Matlab are numbered from 1 upwards.
            if bitget(C, t)
...
% At the moment, the table is in natural binary order because that was the
...
g = g(index + 1, index + 1);% Matlab indices run from 1.

end

% $Id: clifford_sign_table.m 88 2016-07-20 15:33:01Z sangwine $
